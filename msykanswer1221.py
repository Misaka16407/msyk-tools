...
def getHistoryHomework():
    dataup={"studentId":id,"subjectCode":None,"homeworkType":-1,"pageIndex":1,"pageSize":500,"statu":-1,"startTime":None,"endTime":None,"homeworkName":None,"unitId":unitId}
    res = post("https://padapp.msyk.cn/ws/student/homework/studentHomework/getHomeworkList", dataup, 2, "-11500-1")
...
def getSubjectInfo():
    dataup = {"studentId": id, "unitId": unitId}
    res = post("https://padapp.msyk.cn/ws/student/homework/studentHomework/searchSubjectInfo", dataup, 2, " ")
    if res.strip():
        try:
...
def setAccountInform(result):
    ...
    if json.loads(result).get('code') == "10000":
        avatar=json.loads(result).get('InfoMap').get('avatarUrl')
        print(avatar)
...
def getAvgScore():
    dataup = {"subjectCode": None, "groupId": groupId, "stuId": id, "teacherId": None, "startTime": None, "endTime": None, "rows": 25, "pageIndex": 1, "unitId": unitId}
    res = post("https://padapp.msyk.cn/ws/teacher/scoreAnalysis/singleStuScoreGraph", dataup, 2, "251")    
    #print(res)
    if res.strip():
        try:
            reslist = json.loads(res).get('scoreGraphList')  
        except json.JSONDecodeError as e:
            print("JSON格式错误:", e)
    count = 1
    for item in reslist:
        print(
            Fore.RED + str(item['homeworkId']) +
            Fore.YELLOW + " 作业类型:7" + " " + Fore.LIGHTWHITE_EX + 
            str(item['homeworkName']) + Style.BRIGHT + Fore.GREEN + " " + " 作业总分:" + Fore.LIGHTBLUE_EX + 
            str(item['totalScore']) +
            Fore.GREEN + "  我的分数:" + Fore.LIGHTBLUE_EX + str(item['myScore']) + Fore.GREEN + "  平均分:" + Fore.LIGHTBLUE_EX + str(item['groupAvgScore'])
        )
...
def post(url, postdata, type=1, extra=''):
    ...
    if type == 1:
        ...
        key = string_to_md5(str(time) + msyk_key)
...
def ossKey():
    ...
    filename = ""
    filepath = ""
    ossTime = getCurrentTime()
    oss_Key = string_to_md5(str(ossTime) + msyk_key)
    ...
    url = "https://padapp.msyk.cn/ws/common/uploadController/getParams"

    raw = ossPost(url, canshu)
    # print("Raw Response:\n" + raw)

    rt = json.loads(raw)

    if rt.get("code") == "10000":
        print(Fore.GREEN + "Successfully!:")
        print(Fore.YELLOW + "AccessKeyId:     ",Fore.LIGHTWHITE_EX + rt.get("AccessKeyId"))
        print(Fore.YELLOW + "AccessKeySecret: ",Fore.LIGHTWHITE_EX + rt.get("AccessKeySecret"))
        print(Fore.YELLOW + "SecurityToken:   ",Fore.LIGHTWHITE_EX + rt.get("SecurityToken"))
    else:
        print(Fore.RED + "Failed, return code:", rt.get("code"))
    ...
    if str(hwtp) == "7":
        ...
        if len(materialRelasList) == 0:
            ...
            # filepath = input(Fore.RED + "请输入oss文件所在路径: " + Style.RESET_ALL)
            # filemane = input(Fore.RED + "请输入文件名称: " + Style.RESET＿ALL)
        else:
            ...
            for file in materialRelasList:
                ...
                url = file_url
                start_index = url.find("squirrel")
                end_index = url.rfind("/") + 1         #保留最后一个/
                filepath = url[start_index:end_index]
	           # print(filepath)                          
...
#            filepath = input(Fore.RED + "请输入oss文件所在路径: " + Style.RESET_ALL)
#            filemane = input(Fore.RED + "请输入文件名称: " + Style.RESET＿ALL)            
#            oss_tool2.main(filepath,filename)
#            try:
#                oss_tool2.main(filepath,filename)
#            except:
#                print(Fore.RED + f"执行工具时发生错误: {e}")
#                filepath = input(Fore.RED + "请输入oss文件所在路径: " + Style.RESET_ALL)
#                filemane = input(Fore.RED + "请输入文件名称: " + Style.RESET＿ALL)            
#                oss_tool2.main(filepath,filename)
        oss_tool2.main(filepath,filename)
        ...
        passwordTry.passwordTryTest()
    elif Mission == "5":
        passwordTry.passwordTryMain()
    elif Mission == "6":
        getAvgScore()
    # 取消以下注释以启用消息系统和学习圈系统
    # elif Mission == "5":
...
def print_homework_item(item, timePrintStart, timePrintEnd):
    """作业项打印函数"""
...
for item in reslist:
    if str(item['homeworkType']) == '7':
        ...
        timePrintEnd = time.strftime("%Y-%m-%d %H:%M:%S", timeArrayEnd)
        timePrintStart = time.strftime("%Y-%m-%d %H:%M:%S", timeArrayStart)
        print_homework_item(item, timePrintStart, timePrintEnd)
